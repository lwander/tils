...
tils_conn_t *tils_conn_buf_push(tils_conn_buf_t *conn_buf, int client_fd, char *addr_buf) {
    ...
    if (conn_buf->size == conn_buf->conn_cnt)
    ...
    conn_buf->end = TILS_CONN_BUF_ELEM_NEXT(conn_buf->end, conn_buf->conn_cnt);
    conn_buf->size++;
...
tils_conn_state tils_conn_buf_pop(tils_conn_buf_t *conn_buf) {
    ...
    conn_buf->start = TILS_CONN_BUF_ELEM_NEXT(conn_buf->start, conn_buf->conn_cnt);
    conn_buf->size--;
...
void tils_conn_buf_at(tils_conn_buf_t *conn_buf, int i, tils_conn_t **conn) {
    if (i >= conn_buf->size)
        ...
        *conn = &conn_buf->conns[TILS_CONN_BUF_ELEM_AT(conn_buf->start + i, conn_buf->conn_cnt)];
}
 ...
 * @param conn_cnt The number of connections that conn_buf should manage.
 */
void tils_conn_buf_init(tils_conn_buf_t **conn_buf, int conn_cnt) {
